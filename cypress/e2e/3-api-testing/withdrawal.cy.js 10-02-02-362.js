...
describe("withdrawal notification", () => {
    ...
    let mPan = '4960090755';  
    let amount = '565703';  
    let timestamp = date.replace(/-/g, "") + '160312';  
    let responseCode = '91';  
    let variant = 'WITHDRAWAL';  
    let hPan = contractCode + '|' + mPan;  
    let searchKey = date.replace(/-/g, "") + mti + mPan.substring(0, 5) + mPan.substring(mPan.length - 4) + terminalId + rrn + amount;  
    ...
    before("settlement banking, filter", () => {
        cy.request({
            method: "GET",
            url: `http://127.0.0.1:8000/api/settlement/batches`,
            qs: {"stan": stan, "amount": amount }
        }).then((response) => {
            expect(response.status).to.eq(200);
    ...
    it("settlement banking, ID retrieve", () => {
        cy.fixture('monnify_agency').then((monnify_json) => {
            const settlement = monnify_json.settlement;
            settlement.forEach(row => {
                cy.request({
                method: "GET",
                url: `http://127.0.0.1:8000/api/settlement/entries/${row.id}`
                }).then((response) => {
                    expect(response.status).to.eq(200);
                    cy.log("Settlement API response:", response.body);

                    // Overwrite the fixture with the latest response
                    cy.writeFile('cypress/fixtures/settlement_entries.json', response.body);
                })
